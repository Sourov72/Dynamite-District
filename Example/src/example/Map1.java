...
public class Map1 {
    ...
    public Map1(double absolutePosX, double absolutePosY, double degree) {
        ...
        /** If your program doesn't run, change in two places
         * 1. change the image url below to your destination image url
         *2. change the file path in  get map layout to your destination layout
         */
    ...
    public void mapTraverse(double degree, String keyPressed)
    {
        ...
        if((tileLayer2[(int) Math.floor(playerPosY)][(int) Math.floor(playerPosX + 0.5)] != 0|| (tileLayer2[(int) Math.floor(playerPosY)][(int) Math.floor(playerPosX - 0.5)] != 0))) {
            //
            isXObstructed = true;
            //System.out.println("Made true in x tile " + tileLayer2[(int) Math.floor(playerPosX + 1)][(int) Math.floor(playerPosY)] + tileLayer2[(int) Math.floor(playerPosX - 1)][(int) Math.floor(playerPosY)]);
        }
        else {
            isXObstructed = false;
            //System.out.println("Made false in x tile "+ tileLayer2[(int) Math.floor(playerPosX + 1)][(int) Math.floor(playerPosY)] + tileLayer2[(int) Math.floor(playerPosX - 1)][(int) Math.floor(playerPosY)]);
        }
        ...
        if((tileLayer2[(int) Math.floor(playerPosY + 0.25)][(int) Math.floor(playerPosX)] != 0) || (tileLayer2[(int) Math.floor(playerPosY - 0.5)][(int) Math.floor(playerPosX)] != 0)) {
            isYObstructed = true;
            //System.out.println("Made true in y tile " + tileLayer2[(int) Math.floor(playerPosX)][(int) Math.floor(playerPosY + 1)] + tileLayer2[(int) Math.floor(playerPosX)][(int) Math.floor(playerPosY - 1)]);
        }
        else {
            isYObstructed = false;
            //System.out.println("Made false in y tile " + tileLayer2[(int) Math.floor(playerPosX)][(int) Math.floor(playerPosY + 1)] + tileLayer2[(int) Math.floor(playerPosX)][(int) Math.floor(playerPosY - 1)]);
        }
        if (isYObstructed) {
    ...
    public void getMapLayout () {
        ...
        File loadedFile = new File("G:\\Example\\src\\map\\layout1Layer1.txt");
        ...
        loadedFile = new File("G:\\Example\\src\\map\\Layout1Layer2.txt");
        scanner = null;
        try {
            scanner = new Scanner(loadedFile);
        } catch (Exception e) {
